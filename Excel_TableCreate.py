...
class MainApp:
    ...
    def create_table(self):
        ...
        for j in range(0, self.input_rows + 2):    #縦軸に枠線を描画  input_rows + deadline + taskline
            row = self.ref_row + j
            for index in range(0, self.input_columns + self.cell_employee): #横軸に枠線を描画  
                # print(self.column_number + index, row, index)
                cell = self.ws.cell(row, self.column_number + index)
                cell.border = border
                cell.alignment = Alignment(horizontal = 'center',
                                           vertical = 'center',
                                           wrap_text = False)
                    
                if self.deadline_row == row:
                    # deadline_line
                    deadline_color = openpyxl.styles.PatternFill(fgColor="a7f542", bgColor="a7f542", fill_type="solid")
                    cell.fill = deadline_color
                    if self.column_number == self.column_number + index:
                        cell.value = "deadline"

                elif self.task_row == row:
                    # task_line
                ...
                elif self.column_number == self.column_number + index:
                    # employee_line
                    ...
                    # dv.add(self.ws[str(row),char_to_column])
                    # print(str(row),char_to_column)
                    pass
