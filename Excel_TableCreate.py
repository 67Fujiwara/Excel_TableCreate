...
class WindowController:
    ...
    def create_label(self, text, bg_color, font_color, xpos, ypos, width, height, label_title):
...
class MainApp:
    def __init__(self, root):
        ...
        win_control = WindowController(root, "create_table", self.bg_white)
        ...
        win_control.create_label(self.file_path, self.bg_white, self.font_black, 20, 100, 300, 20, "File_Path")
    ...
    def check_table(self):
        self.ref_column = "K"    #基準横軸
        self.ref_row =40     #基準縦軸
        ...
        self.input_columns = 3   #横軸   
        self.input_rows = 3       #縦軸
        self.cell_employee = 1   #社員ｺｰﾄﾞ用ｾﾙ
        self.cell_task = 2  #期限用ｾﾙ + ﾀｽｸ用ｾﾙ  
        return self.ref_column
     
    def open_win1(self):
        self.check_sub = self.create_sub1("sub1", self.bg_white, "300x300")

    def open_win2(self):
        self.value_sub = self.create_sub2("sub2", self.bg_white, "300x300")


    def create_sub1(self, title, bg_color, win_size):
        sub_win1 = Toplevel(self.root)
        sub_win1.title(title)
        sub_win1.config(bg=bg_color)
        sub_win1.geometry(win_size)
        
        
    def create_sub2(self, title, bg_color, win_size):
        sub_win2 = Toplevel(self.root)
        sub_win2.title(title)
        sub_win2.config(bg=bg_color)
        sub_win2.geometry(win_size)
